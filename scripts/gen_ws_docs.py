...
DESCRIPTIONS: dict[str, str] = {
    ...
    "同一平面可多次上报（擦边往复/曲折路线绕回均如实上报，客户端同平面防抖）。"
    "t_ms 为穿越时刻自己计时器时间线上的总时间。服务端按 settled-event 模型"
    "结算：某平面最后一次穿越后静默期（约 0.5s）无再穿越才广播，有效时刻取"
    "最后一次穿越；低于已结算进度游标的迟到乱序事件直接忽略（画面不回跳）。",
    ...
    ">0 = 穿越方落后，可为负。同一 (hit_seat, level_index, seq) 可能再次收到"
    "（结算后再次穿越的修正/amend）——前端按键覆盖取最新。",
